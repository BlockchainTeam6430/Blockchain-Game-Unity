...
namespace Sisus
{
	...
	public class MouseDownInfo : IDisposable
	{
		...
		public EventType GetEventTypeForMouseUpDetection([NotNull]Event e)
		{
			...
			{
				#if DEV_MODE
				Debug.LogWarning("MouseLeaveWindow event is probably actually MouseUp event so treating it as such.\nWindowRect.MouseIsOver="+ inspector.State.WindowRect.MouseIsOver().ToString() + ", mouseoveredInspectorLeftDuringDrag =" + mouseoveredInspectorLeftDuringDrag+", inspector="+(inspector == null ? "null" : inspector.ToString())+ ", mouseDownOverDrawer="+(mouseDownOverDrawer == null ? "null" : mouseDownOverDrawer.ToString()));
				#endif
